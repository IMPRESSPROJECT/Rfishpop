...
#' @param Bio.years The number of recent years to estimate the mean of M, W and Mat (natural mortality, weight and maturity, see information about such elements in Population.Modeling function).
#' @param Method The procedure to obtain the age vector of weight, natural mortality, selectivity and maturity for each iteration. By default is "mean" which means that the mean of the last "Bio.years or "Fish.years" is used. The alternative option is "own", the user can introduce these vectors in a matrix.
...
#' # If par is not NULL must be something like (assuming that W, M,
#' # Mat and SEL are defined previously).
#' # par=list(); par$W<-W; par$SEL<-SEL; par$Mat<-Mat; par$M<-M
#' # BPR(Pop.Mod,f.grid,plot=c(TRUE,1),Method="own",par=par)
#' #
#' @export
