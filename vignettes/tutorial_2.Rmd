...
```{r,eval=FALSE}
RF(Pop.Mod, 3,3,Method="mean",par=NULL,FM_type="F_BPR",iters=1:2,plot=FALSE,prop=0.4)
```

Below, we provide some examples where several fishery mortalities are computed at the same time.

```{r,eval=FALSE}
RF(Pop.Mod, 3,3,Method="mean",par=NULL,FM_type=c("F_Crash","F_msy"),iters=1:2,plot=FALSE)

RF(Pop.Mod, 3,3,Method="mean",par=NULL,FM_type=c("F_Crash","F_msy","F_0.1","F_max","F_BPR"),iters=1:2,plot=FALSE)

RF(Pop.Mod, 3,3,Method="mean",par=NULL,FM_type=c("F_Crash","F_BPR"),iters=1:2,plot=FALSE,prop = c(0.2,0.4))
```
